...
int utn_pedirNumeroEnteroComoMinimo(int* pRespuesta , char* mensaje  , char* mensajeError , int minimo )
{
	...
	if(pRespuesta != NULL && mensaje != NULL && mensajeError !=NULL)//validamos los parametros
	{
		printf("\n%s", mensaje);//imprimimos el mensaje
		if(pedirDatos(bufferString, sizeof(bufferString)) == 0)//pedimos los datos (en formato string)
		{
			if(validarNumeroEntero(bufferString) == 0)//validamos que ese string sea un numero entero
			{
				*pRespuesta = atoi(bufferString);//lo pasamos a un numero enteto
				...
				if(*pRespuesta >= minimo )//varificamos si ese entero esta entre los parametros de maximo y minimo
				{
					retorno = 0;//todo salio ok
...
int utn_pedirLetra(char* pLetra , char* mensaje , char* mensajeError)
{
	...
	{
		printf("\nADVERTENCIA ->solo se va tomar como letra el primer caracter");
		printf("%s",mensaje);
		fflush(stdin);
